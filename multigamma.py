...
def main():
    ...
    try:
        ...
        return
    print(f'Reading the shapefile {shapefile_path}')
    shapefile = ogr.Open(shapefile_path,0)
    layer = shapefile.GetLayer()
    src_srs = layer.GetSpatialRef()

    print(f"Reading the raster data {input_path}")
...
# Get the bounding box of the feature
        ...
        gdal.Warp('test.tif', raster, options=options)
        clipped_raster = gdal.Open('test.tif') 
    ...
    point_ds = driver.CreateDataSource(point_shapefile_path)
