...
# turn on GDAL exceptions to avoid deprecation warning
    ...
    input_ds = driver.Open(input_shapefile, 0)  # 0 means read-only
    if not input_ds:
        raise RuntimeError(f"Unable to open input shapefile: {input_shapefile}")
    ...
    sql_query = f"(BC > 0.55 and (threshold > {threshold_l3} and threshold <  {threshold_u3})) or (BC < 0.55 and (threshold > {threshold_l25} and threshold < {threshold_u25}))"
